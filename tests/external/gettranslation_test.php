...
final class gettranslation_test extends base_external {
     ...
     *
     * @covers \local_deepler\external\get_translation::execute
     * @return void
     ...
     * @throws \invalid_parameter_exception
     */
    public function test_execute_deepl_exception(): void {
        $this->resetAfterTest(true);

        $translations = [['text' => 'Hello', 'source_lang' => 'EN', 'key' => 'greeting']];
        $options = [
                'target_lang' => 'DE',
                'context' => 'general',
                'tag_handling' => 'html',
        ...
        $translatormock = $this->createMock(DeepLClient::class);
        $translatormock->method('translateText')->will($this->throwException(new DeepLException('Test exception')));

        // Injecting mock into get_translation class.
        get_translation::set_translator($translatormock);
