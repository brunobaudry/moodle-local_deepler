...
/*    const hideErrorMessage = (key) => {
    ...
    const toggleAllCheckboxes = (checked) => {
        ...
        requestAnimationFrame(() => {
            updates.forEach(({checkbox, shouldCheck}) => {
                ...
                const row = checkBoxParentMap.get(checkbox).getAttribute('data-key');
                toggleStatus(row, shouldCheck);
            });
    ...
    const toggleChildCheckBoxSelection = (row, shouldBeChecked)=>{
        // Const single = formQuery(Selectors.editors.multiples.checkBoxesWithKey, key);
        const single = formQuery(Selectors.actions.checkBoxes, '', row);
        ...
        allRows.forEach(
            (row)=>{
                ...
                Translation.initTemp(
                    ...
                    editor.editor,
                    editor.editorType,
                    formQuery(Selectors.sourcetexts.singlelangkeys, key, row).nodeValue,
                    formQuery(Selectors.sourcetexts.multilangskeys, key, row).nodeValue
                );
            }
        );
        debouncedShowRows();
